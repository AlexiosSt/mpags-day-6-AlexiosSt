...
ProgramSettings processCommandLine(const std::vector<std::string>& cmdLineArgs){
    ...
    for (std::size_t i{1}; i < nCmdLineArgs; ++i) {
        if (cmdLineArgs[i] == "-h" || cmdLineArgs[i] == "--help") {
            ...
            if (i == nCmdLineArgs - 1) {
                ...
                // We should check that the string contains a valid positive integer.
                const std::string& arg{cmdLineArgs[i + 1]};
                if (arg.front()=='-'){
                    throw InvalidArgument{"--multi-cipher requires a positive integer argument. "+arg+" was the string supplied."};
                }
                try{
            ...
            if (i == nCmdLineArgs - 1) {
                ...
                if (cmdLineArgs[i + 1] == "caesar") {
                    ...
                    throw UnkownArgument{"unkown cipher '"+cmdLineArgs[i+1]+"' was provided"};
                }
                ++i;
            }
        } else {
            ...
            throw UnkownArgument{"unkown argument '"+cmdLineArgs[i]+"' was provided"};
        }
    }
    ...
    if (nTypes != nExpectedCiphers || nKeys != nExpectedCiphers) {
        ...
        errorMsg<<"expected "<<nExpectedCiphers<< " ciphers, but received "<<
            nTypes<<" types and " <<nKeys<<" keys.";
